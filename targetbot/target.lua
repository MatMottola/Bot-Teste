...
TargetBot.saySpell = function(text, delay)
  ...
  if config.ignoreSafety or lastSpell + delay < now then
    say(text)
    lastSpell = now
    return true
  end
...
TargetBot.sayAttackSpell = function(text, delay, ignoreSafety)
  if type(text) ~= 'string' or text:len() < 1 then return end
  if not delay then delay = 2000 end
  if ignoreSafety or lastAttackSpell + delay < now then
    say(text)
    lastAttackSpell = now
    return true
  end
