...
function mwAllowed(tile)
    local topThing = tile:getTopThing()
    local topThingId = topThing:getId()
    return tile:isWalkable() or #tile:getCreatures() > 0 or
        topThingId == 2128 or topThingId == 2129 or
        topThingId == 2130
end

function realNameFromStr(name)
    return name:split(" | ")[1]
end

function getRealName(creature)
    return realNameFromStr(creature:getName())
end
...
onTalk(function(name, level, mode, text, channelId, pos)
    if name ~= getRealName(player) then return end
...
function isBlackListedPlayerInRange(range)
    ...
    for _, spec in pairs(getSpectators()) do
        if spec:isPlayer() and distanceFromPlayer(spec:getPosition()) < range then
            if table.find(storage.playerList.blackList, spegetRealName(c)) then
                safe = true
            end
        end
    end
...
function isSafe(range, multifloor, padding)
    ...
    for _, spec in pairs(getSpectators(multifloor)) do
        if spec:isPlayer() and not spec:isLocalPlayer() and not isFriend(spegetRealName(c)) then
...
function reopenPurse()
    for i, c in pairs(getContainers()) do
        if getRealName(c):lower() == "loot bag" or getRealName(c):lower() == "store inbox" then
            g_game.close(c)
        end
    end
    ...
    schedule(1400, function() 
        for i, c in pairs(getContainers()) do
            if getRealName(c):lower() == "store inbox" then
    ...
    for i, spec in pairs(getSpectators(param1, param2)) do
        if spec ~= player then
            ...
            if spec:isMonster() and (g_game.getClientVersion() < 960 or spec:getType() < 3) then
                monsters = monsters + 1
            elseif spec:isPlayer() and not isFriend(spegetRealName(c)) then
                players = players +1
            end
        end
    end
...
function getContainerByName(name)
    ...
    local d = nil
    for i, c in pairs(getContainers()) do
        if getRealName(c):lower() == name:lower() then
            d = c
            break
        end
    end
...
function createExivaWindow()
    exivaWindow = g_ui.createWidget('ExivaWindow', modules.game_interface.getRootPanel())
    exivaWindow:setFocusable(false)
    exivaWindow:setDraggable(false)
    exivaWindow:setPhantom(true)
    return exivaWindow
end

function updateExivaWindows()
    local gameMap = modules.game_interface.getMapPanel()
    local gameMapX = gameMap:getX()
    local gameMapY = gameMap:getY()
    local gameMapH = gameMap:getHeight()
    local gameMapW = gameMap:getWidth()
    for name, info in pairs(EXIVA_PANELS) do
        if not info.panel then
            EXIVA_PANELS[name].panel = createExivaWindow()
        end
        if info.pos and info.pos.x and info.pos.y and info.pos.z then
            local targetPos = info.pos
            local distance = getDistanceBetween(pos(), targetPos)
            if distance >= 7 and getRealName(player) ~= name and distance <= 500 then
                local zDistance = pos().z - targetPos.z
                local composedText = name .. "(" .. distance
                if zDistance ~= 0 then
                    composedText = composedText .. "/" .. zDistance
                end
                composedText = composedText .. ")"
                EXIVA_PANELS[name].panel.label:setText(composedText)
                local scale = 1
                if math.abs(targetPos.y - posy()) > math.abs(targetPos.x - posx()) then
                    scale = gameMapH/2/math.abs(targetPos.y - posy())
                else
                    scale = gameMapW/2/math.abs(targetPos.x - posx())
                end
                local screenPos = {x=(gameMapX+gameMapW/2)+(targetPos.x - posx())*scale, y=(gameMapY+gameMapH/2)+(targetPos.y - posy())*scale}
                labelW = EXIVA_PANELS[name].panel.label:getWidth()
                labelH = EXIVA_PANELS[name].panel.label:getHeight()
                if screenPos.x > gameMapX + gameMapW - labelW - 50 then
                    screenPos.x = gameMapX + gameMapW - labelW - 50
                elseif screenPos.x < gameMapX then
                    screenPos.x = gameMapX
                end
                if screenPos.y > gameMapY + gameMapH - labelH - 50 then
                    screenPos.y = gameMapY + gameMapH - labelH - 50
                elseif screenPos.y < gameMapY then
                    screenPos.y = gameMapY
                end
                EXIVA_PANELS[name].panel:setPosition(screenPos)
            else
                EXIVA_PANELS[name].panel:destroy()
                EXIVA_PANELS[name].panel = nil
            end
        else
            EXIVA_PANELS[name].panel:destroy()
            EXIVA_PANELS[name].panel = nil
        end
        if os.time() - info.lastUpdate >= 5 and EXIVA_PANELS[name].panel then
            EXIVA_PANELS[name].panel:destroy()
            EXIVA_PANELS[name].panel = nil
        end
    end
