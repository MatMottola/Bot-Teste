...
for i, healingInfo in ipairs({storage.hpitem1, storage.hpitem2, storage.manaitem1, storage.manaitem2}) do
  local healingmacro = macro(20, function()
    ...
    if healingInfo.max >= hp and hp >= healingInfo.min and os.time() > PUSH_TIMEOUT_UNTIL then
      if TargetBot then
...
macro(20, "mana shield", function() 
  if hasManaShield() or lastManaShield + 80000 > now then return end
  if TargetBot then 
...
macro(500, "haste", function() 
  ...
  if os.time() > PUSH_TIMEOUT_UNTIL then
    if TargetBot then 
